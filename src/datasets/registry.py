...
def split_train_into_train_val(dataset, new_dataset_class_name, batch_size, num_workers, val_fraction, max_val_samples=None, seed=0, subset_data_ratio=0.5):
    ...
    # Apply subset ratio to the training set
    if subset_data_ratio < 1.0:
        random.seed(42)
        print(f"Subset data ratio: {subset_data_ratio}")
        subset_indices = random.sample(range(len(trainset)), int(subset_data_ratio * len(trainset)))
...
def get_dataset(dataset_name, preprocess, location, batch_size=128, num_workers=0, val_fraction=0.1, max_val_samples=5000):
    if dataset_name.endswith('Val'):
        ...
        if dataset_name in registry:
            ...
            dataset = split_train_into_train_val(
                base_dataset, dataset_name, batch_size, num_workers, val_fraction, max_val_samples, subset_data_ratio=0.01)
            return dataset
    else:
    ...
    dataset = dataset_class(
        preprocess, location=location, batch_size=batch_size, num_workers=num_workers, subset_data_ratio=0.01
    )
    return dataset
