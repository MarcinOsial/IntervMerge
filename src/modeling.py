...
class ImageClassifier_original(torch.nn.Module):
...
class ImageClassifier(torch.nn.Module):
    ...
    def forward(self, inputs, interventions):
        features = self.image_encoder(inputs, interventions)
        outputs = self.classification_head(features)
        return outputs

    def __call__(self, inputs, interventions):
        return self.forward(inputs, interventions)
